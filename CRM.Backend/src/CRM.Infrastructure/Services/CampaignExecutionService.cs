...
public class CampaignExecutionService : CRM.Core.Interfaces.ICampaignExecutionService
{
    ...
    /// <summary>
    /// Get a specific campaign workflow by ID
    /// </summary>
    public async Task<CampaignWorkflow?> GetCampaignWorkflowByIdAsync(int campaignWorkflowId)
    {
        return await _context.CampaignWorkflows
            .Include(cw => cw.WorkflowDefinition)
            .FirstOrDefaultAsync(cw => cw.Id == campaignWorkflowId && !cw.IsDeleted);
    }
