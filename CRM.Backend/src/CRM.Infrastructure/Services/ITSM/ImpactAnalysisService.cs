...
public class ImpactAnalysisService : IImpactAnalysisService
{
    ...
    public async Task<IncidentImpactAnalysis> AnalyzeIncidentImpactAsync(int incidentId)
    {
        ...
        {
            affectedCI = await _context.ConfigurationItems
        ...
        if (incident.ConfigurationItemId.HasValue)
        {
            analysis.PotentialRootCauses = await FindPotentialRootCausesAsync(incident.ConfigurationItemId.Value, _context);
        }
    ...
    public async Task<DependencyChain> GetDependencyChainAsync(int configurationItemId)
    {
        ...
        chain.DependsOn = await GetUpstreamDependenciesAsync(configurationItemId, _context, 1, 3);

        // Get downstream dependencies (what depends on this CI)
        chain.DependedOnBy = await GetDownstreamDependenciesAsync(configurationItemId, _context, 1, 3);
        ...
        int ciId, ICrmDbContext context, int currentLevel, int maxLevel)
    {
        ...
        var relationships = await _context.CIRelationships
        ...
        foreach (var rel in relationships)
        {
            ...
            node.Children = await GetUpstreamDependenciesAsync(
                rel.ParentCIId, _context, currentLevel + 1, maxLevel);
        ...
        int ciId, ICrmDbContext context, int currentLevel, int maxLevel)
    {
        ...
        foreach (var rel in relationships)
        {
            ...
            node.Children = await GetDownstreamDependenciesAsync(
                rel.ChildCIId, _context, currentLevel + 1, maxLevel);
    ...
    private async Task<List<PotentialRootCause>> FindPotentialRootCausesAsync(int ciId, ICrmDbContext context)
    {
        ...
        var upstreamCIs = await _context.CIRelationships
            .Include(r => r.ParentCI)
            .Where(r => r.ChildCIId == ciId && r.RelationshipType == CIRelationType.DependsOn)
            .Select(r => r.ParentCI)
            .ToListAsync();
        ...
        foreach (var upstreamCI in upstreamCIs.Where(ci => ci != null))
        {
            ...
            var recentChanges = await _context.Changes
