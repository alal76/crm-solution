...
{
    /// <summary>
    /// Start the KCS workflow for an incident (capture in _context).
    /// </summary>
...
public class KCSWorkflowService : IKCSWorkflowService
{
    ...
    public async Task<KCSReviewRequest> SubmitForReviewAsync(int articleId, int submittedById)
    {
        ...
        var submitter = await _context.Users.FindAsync(submittedById);
        ...
        string? feedback = null)
    {
        ...
        var article = await _context.ITSMKnowledgeArticles.FindAsync(request.ArticleId);
    ...
    public async Task<List<KCSCoachingItem>> GetCoachingQueueAsync(int reviewerId)
    {
        ...
        var newAuthorArticles = await _context.ITSMKnowledgeArticles
            .Include(a => a.Author)
