...
public class AllenAIService : IAllenAIService
{
    ...
    private decimal CalculateOpportunityHealth(Opportunity opportunity)
    {
        ...
        // Stage progress
        if (opportunity.Stage >= OpportunityStage.Proposal) score += 20;
        else if (opportunity.Stage >= OpportunityStage.Qualification) score += 10;

        // Has expected close date
        if (opportunity.ExpectedCloseDate.HasValue) score += 10;

        // Amount specified
        if (opportunity.Amount > 0) score += 10;

        // Time factor - deduct if too long
        var daysSinceCreation = (DateTime.UtcNow - opportunity.CreatedAt).Days;
        if (daysSinceCreation > 90) score -= 20;
    ...
    {
        if (healthScore < 40) return OpportunityActionType.RiskMitigation;
        if (winProbability >= 0.8m) return OpportunityActionType.FastTrackClose;
        if (winProbability >= 0.6m) return OpportunityActionType.ProposalReview;
    ...
    private decimal CalculateChurnProbability(Account customer)
    {
        ...
        if (monthsSinceCreation < 3) risk += 0.2m;
    ...
    private decimal CalculateCustomerHealth(Account customer)
    {
        ...
        if (score == 0)
        {
            ...
            if (monthsSinceCreation >= 24) score += 10;
        }
    ...
    private EmailSentiment ExtractSentiment(string response)
    {
        ...
        if (lower.Contains("very positive")) return EmailSentiment.VeryPositive;
        if (lower.Contains("very negative")) return EmailSentiment.VeryNegative;
        if (lower.Contains("positive")) return EmailSentiment.Positive;
        if (lower.Contains("negative")) return EmailSentiment.Negative;
    ...
    {
        var lower = response.ToLower();
        if (lower.Contains("purchase") || lower.Contains("buy")) return EmailIntent.PurchaseIntent;
        if (lower.Contains("support") || lower.Contains("help")) return EmailIntent.SupportRequest;
        if (lower.Contains("complaint") || lower.Contains("unhappy")) return EmailIntent.Complaint;
        if (lower.Contains("meeting") || lower.Contains("schedule")) return EmailIntent.MeetingRequest;
        if (lower.Contains("question") || lower.Contains("inquiry")) return EmailIntent.Inquiry;
        if (lower.Contains("feedback")) return EmailIntent.Feedback;
        if (lower.Contains("follow")) return EmailIntent.FollowUp;
    ...
    {
        var lower = response.ToLower();
        if (lower.Contains("immediate") || lower.Contains("urgent")) return ResponseUrgency.Immediate;
        if (lower.Contains("high")) return ResponseUrgency.High;
        if (lower.Contains("low")) return ResponseUrgency.Low;
        if (lower.Contains("no response")) return ResponseUrgency.NoResponse;
