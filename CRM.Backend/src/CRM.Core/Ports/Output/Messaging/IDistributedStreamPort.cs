...
/// <summary>
/// Technology-neutral distributed messaging and cache port interface.
/// Extends the base IMessageQueueProvider with pub/sub, streaming,
/// queuing, caching, hash, and distributed-lock operations.
/// Implementations may use Redis, Valkey, NATS, or other backends.
/// </summary>
public interface IDistributedStreamPort : IMessageQueueProvider
{
    #region Pub/Sub

    /// <summary>
    /// Publishes a message to a channel (Pub/Sub).
    /// </summary>
    Task<long> PublishToChannelAsync<T>(string channel, T message, CancellationToken cancellationToken = default) where T : class;

    /// <summary>
    /// Subscribes to a channel (Pub/Sub).
    /// </summary>
    ...
    /// </summary>
    Task<DistributedStoreServerInfo> GetServerInfoAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets memory usage statistics.
    /// </summary>
    Task<DistributedStoreMemoryStats> GetMemoryStatsAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets cluster information (if in cluster mode).
    /// </summary>
    Task<DistributedStoreClusterInfo?> GetClusterInfoAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Pings the backend store.
    /// </summary>
    ...
    /// <summary>
    /// Flushes the current database/namespace.
    /// </summary>
...
#region Distributed Store Types
...
{
    /// <summary>Maximum length of the stream (auto-trim).</summary>
    public long? MaxLength { get; init; }

    /// <summary>Whether to use approximate trimming.</summary>
...
/// <summary>
/// Distributed store server information.
/// </summary>
public record DistributedStoreServerInfo(
    string Version,
    string Mode,
...
/// <summary>
/// Distributed store memory statistics.
/// </summary>
public record DistributedStoreMemoryStats(
    long UsedMemory,
    long UsedMemoryRss,
    long UsedMemoryPeak,
    long UsedMemoryLua,
...
/// <summary>
/// Distributed store cluster information.
/// </summary>
public record DistributedStoreClusterInfo(
