...
//
// ARCHITECTURE NOTE:
// These constants define the available provider implementations for each category.
// Used in configuration to specify which provider to use when external providers are enabled.
// Configuration key: Providers:{Category}:Type
//
// Pattern: [Feature Flag = true] → [ProviderTypes.{Category}.{Type}] → [Provider Implementation]
