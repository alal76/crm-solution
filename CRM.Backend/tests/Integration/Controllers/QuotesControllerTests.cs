...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class QuotesControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_Quotes_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/quotes");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/quotes should not return a server error");
        }
