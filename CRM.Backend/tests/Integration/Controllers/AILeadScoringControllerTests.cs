...
namespace CRM.Backend.Tests.Integration.Controllers
{
    public class AILeadScoringControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_AILeadScoring_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/ai/leads");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/ai/leads should not return a server error");
        }
