...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class ZipCodesControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_ZipCodes_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/zipcodes");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/zipcodes should not return a server error");
        }
