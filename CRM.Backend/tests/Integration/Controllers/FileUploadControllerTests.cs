...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class FileUploadControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_FileUpload_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/fileupload");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/fileupload should not return a server error");
        }
