...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class EmailTemplatesControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_EmailTemplates_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/emailtemplates");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/emailtemplates should not return a server error");
        }
