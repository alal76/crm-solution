...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class PermissionsControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        public async Task GetEndpoint_Permissions_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/permissions");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/permissions should not return a server error");
        }
