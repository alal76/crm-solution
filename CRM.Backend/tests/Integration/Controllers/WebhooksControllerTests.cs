...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class WebhooksControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_Webhooks_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/webhooks");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/webhooks should not return a server error");
        }
