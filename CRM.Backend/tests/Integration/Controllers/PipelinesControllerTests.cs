...
namespace CRM.Backend.Tests.Integration.Controllers
{
    public class PipelinesControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_Pipelines_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/pipelines");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/pipelines should not return a server error");
        }
