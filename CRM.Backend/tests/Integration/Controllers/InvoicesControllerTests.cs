...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class InvoicesControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_Invoices_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/invoices");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/invoices should not return a server error");
        }
