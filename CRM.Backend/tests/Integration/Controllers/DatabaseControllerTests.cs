...
namespace CRM.Backend.Tests.Integration.Controllers
{
    ...
    public class DatabaseControllerTests : IClassFixture<ApiTestFactory>
    {
        ...
        [Fact]
        public async Task GetEndpoint_Database_ReturnsNon500()
        {
            var res = await _client.GetAsync("/api/database");
            ((int)res.StatusCode).Should().BeLessThan(500, "GET /api/database should not return a server error");
        }
