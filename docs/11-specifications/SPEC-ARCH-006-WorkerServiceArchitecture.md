...
|----|-------------|-------------|--------|
| WS-001 | Worker Host | Dedicated process for background jobs | ⚠️ |
| WS-002 | Queue/Backbone | Transport and job routing | ❌ |
| WS-003 | Outbox Dispatcher | Durable event emission | ⚠️ |
...
| Service | File Path | Methods | Status |
|---------|-----------|---------|--------|
| WorkerHost | `CRM.Infrastructure/Workers/WorkerHost.cs` | 5 | ⚠️ | Host loop does not dequeue work yet |
| EscalationWorker | `CRM.Infrastructure/Workers/EscalationWorker.cs` | 6 | ⚠️ | Processor wiring pending real logic |
| NotificationWorker | `CRM.Infrastructure/Workers/NotificationWorker.cs` | 6 | ⚠️ | Uses dispatcher but no queue integration |
| OutboxDispatcher | `CRM.Infrastructure/Integration/OutboxDispatcher.cs` | 5 | ⚠️ | Database persistence in place |
...
|------|-------------------|--------|---------|
| Worker host processing loop | `CRM.Infrastructure/Workers/WorkerHost.cs` | No dequeue/dispatch logic | TODO-ARCH-006-006 |
| Queue backbone integration | `CRM.Infrastructure/Workers/DbWorkerQueue.cs` | DB queue only | TODO-ARCH-006-007 |
| Observability & DLQ | `CRM.Infrastructure/Workers/` | Metrics/logs/retry policies missing | TODO-ARCH-006-008 |
...
| TODO-ARCH-006-006 | Implement worker host dequeue/dispatch loop | P0 | Backend |
| TODO-ARCH-006-007 | Add queue backbone (Redis/RabbitMQ) | P1 | Architecture |
| TODO-ARCH-006-008 | Add escalation and notification job processors | P0 | Backend |
