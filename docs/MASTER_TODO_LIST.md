...
> **Last Updated:** August 10, 2026 (full end-to-end review — see [Section 2A](#section-2a--engineering-review-findings-august-6-2026) — followed by nine rounds of remediation work on `tech-debt-cleanup-aug2026`, see [Section 2B](#section-2b--findings-discovered-while-implementing-section-2a-august-6-2026); round 4 closed all 5 previously decision-gated items; round 5 was a documentation-accuracy + StyleCop cleanup pass; round 6 closed REV-DOC-005's remaining test-coverage gaps, REV-FE-004/007, REV-STUB-001, REV-ORPHAN-005, and REV-BUG-006's outbound Stripe half; round 7 closed the remaining Section 2A decision-driven batch; round 8 closed everything else still open in Section 2A — REV-FE-003 (new ReportTemplate backend), REV-STUB-012/013 (real DunningManager Stripe charges), the REV-FE-005/006/008 stale-finding corrections, and a full REV-STUB-014 investigation confirming all 29 archived files are genuinely dead; round 9 closed REV-ORPHAN-006, formalizing and resolving all ~12 orphaned services flagged by round 8's investigation)
> **Version:** 0.628.41 (branch `tech-debt-cleanup-aug2026`, not yet merged to `main`)
...
| REV-ORPHAN-006 | ✅ **Closed** (`tech-debt-cleanup-aug2026`, round 9). Formalizes and resolves the round-8 REV-STUB-014 side finding: ~12 active ITSM/commission services DI-registered and tested but unreached by any controller. Investigation surfaced 3 more hidden competing-implementation situations beyond the original finding's assumption (matching the REM-ORPHAN-001/002 pattern) — each verified before touching code, none wired blindly: <br>• **`CommissionRuleService`** — CRUD didn't compete with `ICommissionRulesEngine` (calculation-only, round-4 REM-ORPHAN-002 winner); only `CalculateCommissionAsync` marked `[Obsolete]`, CRUD wired into `CommissionsController` (`api/commissions/rules[/{id}]`). <br>• **`DiscountRuleService`** — fully redundant with the already-wired `PricingRulesController`/`IPricingRulesService`/`IDynamicPricingEngine` (`PricingRule` is a strict superset of `DiscountRule`'s fields, including its own `DiscountMethod`). Whole class marked `[Obsolete]`, not wired (matches the `TerritoryAssignmentService` whole-class precedent). <br>• **`ChangeCalendarService`** — blackout-period methods duplicated `IChangeManagementServiceEx`'s real DB-backed blackout data (round-4 REM-ORPHAN-001 winner) with a hardcoded fake list; those 5 methods marked `[Obsolete]`. Its genuinely novel `FindAvailableSlotsAsync`/`GetMaintenanceWindowsAsync` reworked to consume real blackout/change data via the injected `IChangeManagementServiceEx`, then wired into `ChangesController`. <br>• **`KCSWorkflowService.PublishArticleAsync`** — duplicated `KnowledgeController`'s existing publish endpoint; marked `[Obsolete]`, left unwired. The rest of `KCSWorkflowService` (capture session, draft-from-incident, review workflow, agent metrics, lifecycle, flagging, coaching queue) is novel and was wired. <br>Safe wire-ups with no competing implementation, verified via full method-surface comparison before wiring: `AssetLifecycleService` + `DiscoveryService` → `CMDBController`; `CatalogApprovalService` + `CatalogFulfillmentService` → `CatalogController`; `ChangeImpactService` → `ChangesController`; `AssignmentRulesEngine` + `ImpactAnalysisService` → `IncidentsController` (accepted as a second, separately-modeled assignment engine alongside the existing ServiceRequest-scoped `AutoAssignmentService` — a real gap, not a duplicate, since Incidents previously had zero rule-based assignment); `ArticleRecommendationService` → `KnowledgeController`. 131 new tests across 7 commits, all independently verified (diff review + fresh build/test, not just trusting subagent reports) before committing. Full 3-project backend suite re-verified green after all 7 workstreams: 10,927 tests passing, 0 failures. | SPEC-SLS-006-CommissionTerritoryManagement.md |
