...
> **Last Updated:** August 6, 2026 (full end-to-end review — see [Section 2A](#section-2a--engineering-review-findings-august-6-2026) — followed by two rounds of remediation work on `tech-debt-cleanup-aug2026`, see [Section 2B](#section-2b--findings-discovered-while-implementing-section-2a-august-6-2026))
> **Version:** 0.626.7 (branch `tech-debt-cleanup-aug2026`, not yet merged to `main`)
...
> **Active Backlog:** 1 blocked (INT-003) + 1 deferred by architectural decision (XMOD-011) + Section 2A (27 items — most fixed on `tech-debt-cleanup-aug2026`, see status markers) + Section 2B (25 items found while implementing 2A — 13 fixed, 5 partial, 7 open with a full remediation plan; the 5-instance "tested service the live API never calls" pattern, REV-ORPHAN-\*, is only 1/5 resolved and 3 of the remaining 4 need human sign-off before an agent should touch them) + **6 new REV-FGAP-003–008 field-gap findings from completing the FIELD_GAP_REMEDIATION_PLAN.md re-audit (REV-DOC-003 now fully closed — all 16 entities re-verified)**
> **Build:** ✅ 0 backend build errors across **both** backend test projects (`tests/CRM.Tests.csproj` and `tests/CRM.Tests/CRM.Tests.csproj` — this repo has two, don't check only one) | ⚠️ 9 live StyleCop SA warnings (42 total incl. non-SA), down from 255/286 | ✅ Frontend `tsc --noEmit` clean, `CI=true npm run build` succeeds, full Jest suite green | **Tests:** ✅ 2,877 + 7,495 = **10,372 backend tests passing, 0 failures** (re-verified `--no-incremental`, Aug 6 2026) | ✅ **1,118 frontend tests passing, 0 failures** (44 suites) | **Coverage:** ~70% (TCOV-001–068 done) — still overstates method-level completeness for AccountService/LeadsController/WorkflowController/AllenAIService (unaddressed this pass) | **Next:** work through the Section 2B remediation plan, starting with the no-decision-needed items (REM-FGAP-004/005/007/008, REM-BUG-007 territoryService half, REM-BUG-009 test-coverage half, REM-ORPHAN-005 caller wiring)
>
> **⚠️ Process note for future sessions on this branch:** during the second remediation round, several subagents reported work as "completed" that had actually been silently lost or reverted mid-session (`TasksController.cs`, `FeatureFlagsPanel.tsx`, `IntegrationsSettingsPage.tsx`, and the `OrderService`/`ContactsService`/`MergeService` fixes all had to be re-verified against disk and, in most cases, redone) — most likely from concurrent agents racing on the same working tree combined with an environment auto-commit/checkpoint feature that fires mid-session outside normal `git commit` calls (confirmed: commit `972eda3d` was not authored by any explicit commit in this session). **Do not trust an agent's self-report of "done" on this branch without independently re-reading the file and re-running the build/tests** — this cost significant rework time. Consider `isolation: 'worktree'` for parallel agents touching overlapping files next time.
...
| REV-ORPHAN-005 | ⚠️ **Partially fixed** (`tech-debt-cleanup-aug2026`). `ISlackNotificationService`/`ITeamsNotificationService`/`ITwilioCallLoggingService` are now registered in DI (Scoped), correctly distinguished from decoy same-named classes in a different namespace. Still open: no caller wired for Slack/Teams (needs a design decision on where to hook them into business logic — escalation? workflow steps?); no caller wired for Twilio either, because no call-status-callback webhook endpoint exists yet (`TwilioWebhookController` only handles SMS status callbacks) — inventing one was judged out of scope for this pass. | Commits on `tech-debt-cleanup-aug2026`, this section |
...
| REV-BUG-007 | `territoryService.ts`'s "Auto-Assign" button calls a route that doesn't exist on the backend. Three separate frontend `OrderStatus` enums exist, one (`types/sales.ts`) with numeric values that don't match the backend at all — contradicts `FIELD_GAP_REMEDIATION_PLAN.md`'s "Complete"/"mapped" claim for Order. | ⚠️ **Partially fixed** on `tech-debt-cleanup-aug2026` — the Order-enum half is done (consolidated to one canonical `OrderStatus` matching the backend, `orderService.ts`/`OrdersPage.tsx` import from it). The `territoryService.ts` dead-route half was not addressed in this pass. |
...
| REV-BUG-009 | `Contact`: `PreferredContactMethod` is accepted in request DTOs but silently dropped — never written to the entity. `Update`/`Delete`/social-media endpoints have zero unit-test coverage at both service and controller layers. Account-assignment methods exist on `ContactsService` but have no controller route. | ⚠️ **Partially fixed** on `tech-debt-cleanup-aug2026` — `PreferredContactMethod` (and the equally-missing `DoNotContact` on update) now persist on create/update, with regression tests. The test-coverage and account-assignment-route parts of this finding were not re-verified/addressed in this pass. |
...
| REM-ORPHAN-005 | ⚠️ Partial | P2 | S | DI registration done. Still needed: wire at least one real caller each for Slack/Teams (workflow/escalation notification steps) and Twilio (needs a call-status webhook endpoint that doesn't exist yet). | No — pure wiring, now that REV-STUB-001/009 made the implementations real. |
...
| REM-BUG-007 | ⚠️ Partial | P2 | S | Order-enum-consolidation half done. `territoryService.ts`'s dead "Auto-Assign" route still needs fixing/removing. | No. |
| REM-BUG-008 | ✅ **Fixed** | P2 | S | ~~Add the missing write-DTO fields~~ — done, mirroring the REV-FGAP-002 pattern. | — |
| REM-BUG-009 | ⚠️ Partial | P2 | M | `PreferredContactMethod`/`DoNotContact` persistence done, with regression tests. Missing test coverage on `Update`/`Delete`/social-media endpoints and the orphaned account-assignment route were not addressed in this pass. | No. |
...
| REM-FGAP-003 | ❌ Open | P1 | M | Add the ~20 missing BANT/MEDDIC/UTM/nurture/territory fields to `LeadDto`/`LeadSummaryDto`, mirroring the REV-FGAP-001/002 pattern. Note: this DTO belongs to the orphaned `Lead` system (REV-ORPHAN-003/REM-ORPHAN-003) — low value to fix in isolation until that product decision lands. | Soft — technically no decision needed to add the fields, but consider bundling with REM-ORPHAN-003 rather than fixing a DTO for a system that might be retired. |
| REM-FGAP-004 | ❌ Open | P2 | S | Add the 8 missing fields to `QuoteDto` (`warrantyEndDate`, `termsAndConditions`, `expectedDeliveryDate`, `actualDeliveryDate`, `serviceStartDate`, `serviceEndDate`, `attachments`, `customFields`), mirroring REV-FGAP-001/002. | No. |
| REM-FGAP-005 | ❌ Open | P2 | S | Add the 14 missing fields to the frontend `Payment` type (`FraudFlagged`, `RiskScore`, `AmountApplied`/`AmountUnapplied`, `RefundedAmount`, `RetryCount`, + 9 more — see FIELD_GAP_REMEDIATION_PLAN.md). Check `docs/FIELD_GAP_REMEDIATION_PLAN.md`'s "Intentionally Excluded Fields" section first — some of these (fraud-related) may be deliberately excluded from the frontend, not a gap. | Soft — confirm none of the 14 are deliberately security-excluded before adding them all to the frontend type. |
| REM-FGAP-006 | ❌ Open | P2 | S | Either wire `ContractDto.AnnualValue`/`RenewalTermMonths` to real entity fields (may need new entity fields + migration if they don't exist anywhere) or remove the dead DTO fields so client writes stop being silently discarded. | Soft — depends whether these were meant to be real fields (needs a migration) or should just be removed; a quick product check on whether anyone relies on them. |
| REM-FGAP-007 | ❌ Open | P2 | S | Add `DueDate`, `StatusCode`, `LastModifiedByUserId`, `ConversationId` to `ServiceRequestDto`, mirroring REV-FGAP-001/002. | No. |
| REM-FGAP-008 | ❌ Open | P3 | M | Add `PasswordNeverSet`/`CommissionPlanId` to `UserDto`; add the ~20 missing preference/security fields to the frontend `User` type in `common.ts`; also consolidate the local duplicate `User` type in `UserManagementPage.tsx` into the shared one while touching this. | No. |

**Sequencing:** REM-BUG-006 and REM-ORPHAN-001/002/003 still need explicit sign-off (architecture or product) before an agent should touch them — attempting them blind risks silently deleting the *better* implementation, which would make this exact class of problem worse, not better. REM-BUG-011 needs design input. Everything else above is safe to pick up without a decision; REM-FGAP-004/005/007/008 are the smallest remaining no-decision items.
