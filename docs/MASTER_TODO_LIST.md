...
> **Last Updated:** August 7, 2026 (full end-to-end review — see [Section 2A](#section-2a--engineering-review-findings-august-6-2026) — followed by six rounds of remediation work on `tech-debt-cleanup-aug2026`, see [Section 2B](#section-2b--findings-discovered-while-implementing-section-2a-august-6-2026); round 4 closed all 5 previously decision-gated items; round 5 was a documentation-accuracy + StyleCop cleanup pass; round 6 closed REV-DOC-005's remaining test-coverage gaps, REV-FE-004/007, REV-STUB-001, REV-ORPHAN-005, and REV-BUG-006's outbound Stripe half, plus built (but did not run) a Lead history-continuity tool)
> **Version:** 0.628.19 (branch `tech-debt-cleanup-aug2026`, not yet merged to `main`)
...
> **Active Backlog:** 1 blocked (INT-003) + 1 deferred by architectural decision (XMOD-011) + Section 2A (27 items — most fixed) + Section 2B (25 items found while implementing 2A — **all fixed/wired except one tooling-built-not-run item**) + 6 REV-FGAP-003–008 field-gap findings (**all 6 fixed**). One follow-up deliberately left open: actually running the Lead backfill + history-continuity tools against real data — both are built, tested, and safe-by-default (dry-run), but this repo has a single production database (no staging copy), so execution needs your explicit go-ahead, not an agent's judgment call. REM-BUG-006's Stripe wiring is code-complete but unverified against a live Stripe sandbox (needs your API credentials to close fully).
> **Build:** ✅ 0 backend build errors across **both** backend test projects (`tests/CRM.Tests.csproj` and `tests/CRM.Tests/CRM.Tests.csproj`) | ✅ 0 live StyleCop SA warnings | ✅ Frontend `tsc --noEmit` clean, `CI=true npm run build` succeeds | **Tests:** ✅ 7,533 + 3,044 = **10,577 backend tests passing, 0 failures** (re-verified fresh Aug 7 2026 after round 6 — up from 10,424 due to round 6's new coverage) | ✅ **1,142 of 1,142 frontend tests passing** (47 suites, 0 failures — the previously-flaky `IntegrationsSettingsPage` tests passed clean this run too) | **Coverage:** ~70% (TCOV-001–068 done) — AccountService/LeadsController/WorkflowController/AllenAIService method-level gaps closed in round 6; other classes' TCOV/TCOV2 markers still carry the general "file exists and passes, not thoroughly covered" caveat | **Next:** nothing decision-gated remains in Section 2B; the only open items are the Lead-migration execution go-ahead (REM-LEAD-BACKFILL-EXEC, REM-LEAD-HISTORY-CONTINUITY) and Stripe live-sandbox verification, both needing you, not more agent work
>
...
| REV-STUB-002 | `AccountingSyncService` — QuickBooks/Xero sync all return "not yet implemented" (`Services/Integrations/AccountingSyncService.cs:47,58,69,100`). **Decision recorded (round 6, round 7 to implement):** write real SDK-wired code even though it can't be tested live here; add a credentials-entry UI (none currently exists for third-party integration keys); research whether free/sandbox tiers exist and document signup steps (account creation itself needs the user, not the agent); if no free tier, write in-app setup instructions instead. |
| REV-STUB-003 | `MarketingSyncService` — Mailchimp/HubSpot contact/segment/campaign sync all stubbed "not yet implemented" (`Services/Integrations/MarketingSyncService.cs:49,63,77,93`). **Decision recorded (round 6, round 7 to implement):** same treatment as REV-STUB-002. |
| REV-STUB-004 | `LinkedInSalesNavService` messaging path and `CommunicationService` LinkedIn delivery are stubs separate from INT-003's Sales Navigator scope (`Services/Integrations/LinkedInSalesNavService.cs:22,82`; `Services/CommunicationService.cs:753`). **Decision recorded (round 6, round 7 to implement):** same treatment as REV-STUB-002. |
| REV-STUB-005 | `SchedulingIntegrationService` — stub implementation (`Services/Integrations/SchedulingIntegrationService.cs:21`). **Decision recorded (round 6, round 7 to implement):** same treatment as REV-STUB-002. |
| REV-STUB-006 | `CloudDeploymentService` — AWS/Azure/GCP/DigitalOcean connection tests all "not yet implemented" (`Services/CloudDeploymentService.cs:652,1230,1241,1252,1262`). **Decision recorded (round 6, round 7 to implement):** wire all four providers generically via each SDK's own connection-test call — code will be real, but verification against live accounts needs credentials for all four, which aren't available here. |
| REV-STUB-007 | `GeoLocationService` — GeoIP lookup returns mock data only (`Services/Auth/GeoLocationService.cs:36,52,82,94`, tagged TODO-AUTH-024). **Decision recorded (round 6, round 7 to implement):** same treatment as REV-STUB-002. |
| REV-STUB-008 | `PythonScriptEngine.IsAvailable => false` — Python scripting entirely non-functional pending pythonnet integration (`Infrastructure/Scripting/PythonScriptEngine.cs:16-30`). **Decision recorded (round 6, round 7 to implement):** build a narrow, restricted subset (pure computation only — no filesystem/network access), security model documented explicitly. Adds a new system-level runtime dependency (pythonnet + CPython must be present on every deploy target) — flag this operational requirement clearly when shipped. |
...
| REV-STUB-011 | `CampaignExecutionService.cs:877` — "Stub implementation - returns basic success result". **Decision recorded (round 6, round 7 to implement):** build a fuller campaign engine — scheduling, segment evaluation, throttling, open/click tracking — not just a channel-send wrapper. |
...
| ID | Description |
|----|---|
| REV-FE-001 | `AccountMergeDialog.tsx` / `AccountHierarchyTree.tsx` referenced but don't exist — exports commented out (`components/crm/accounts/index.ts:6-15`). **Decision recorded (round 6, round 7 to implement):** build the full-featured UI — field-by-field conflict resolution merge picker plus a hierarchy tree view. Backend `MergeService` is already real, tested, and working; this is pure UI work. |
| REV-FE-002 | `EmailDigestPage.tsx` entirely non-functional — no backend endpoint; load/save/preview all no-op (`pages/EmailDigestPage.tsx:76-109`). **Decision recorded (round 6, round 7 to implement):** build the full feature (backend scheduling + aggregation + endpoint, real save/preview). Digest content composition (what a "digest" actually contains) was not specified in the decision round — needs a concrete content definition before/during implementation, flagged for confirmation rather than guessed. |
...
**Sequencing:** as of the round-6 pass, all 5 originally decision-gated items are closed, plus REM-ORPHAN-005's Twilio half and REM-LEAD-HISTORY-CONTINUITY's tooling (round 6). The user was asked for a decision on each non-obvious item (via research-backed multiple-choice questions, not a blind agent call) before any code was written — REM-ORPHAN-001/002 were preceded by dedicated comparison research passes; REM-BUG-006's outbound Stripe decision and REM-STUB-006/008/011 and the credential-blocked integration bucket (Section 2A) were likewise decided explicitly before round 7's work starts. What remains genuinely open in Section 2B: REM-LEAD-BACKFILL-EXEC and REM-LEAD-HISTORY-CONTINUITY's actual execution against real data (both tools built/tested/safe-by-default, deliberately not run — single production database, no staging copy, needs your explicit go-ahead), and REM-BUG-006's live-sandbox verification (code-complete, needs your real Stripe credentials to verify end-to-end).
