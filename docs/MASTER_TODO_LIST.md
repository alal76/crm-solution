...
> **Last Updated:** August 7, 2026 (full end-to-end review — see [Section 2A](#section-2a--engineering-review-findings-august-6-2026) — followed by five rounds of remediation work on `tech-debt-cleanup-aug2026`, see [Section 2B](#section-2b--findings-discovered-while-implementing-section-2a-august-6-2026); round 4 closed all 5 previously decision-gated items after the user made the calls on each; round 5 was a documentation-accuracy + StyleCop cleanup pass)
> **Version:** 0.628.10 (branch `tech-debt-cleanup-aug2026`, not yet merged to `main`)
...
> **Active Backlog:** 1 blocked (INT-003) + 1 deferred by architectural decision (XMOD-011) + Section 2A (27 items — most fixed) + Section 2B (25 items found while implementing 2A — **24 fixed/wired, 1 mostly-fixed** — the 5 formerly decision-gated items are now all closed) + 6 REV-FGAP-003–008 field-gap findings (**all 6 fixed**). Two follow-ups deliberately left open, documented below: actually running the Lead backfill against real data (tooling built, dry-run only), and re-parenting old Contact-based Activities/Comments onto migrated Lead IDs.
> **Build:** ✅ 0 backend build errors across **both** backend test projects (`tests/CRM.Tests.csproj` and `tests/CRM.Tests/CRM.Tests.csproj` — this repo has two, don't check only one) | ✅ **0 live StyleCop SA warnings** (round 5 — was 9 as of round 3, grew to 12 unique/24 total from round-4 additions before this cleanup; the previous "needs individual judgment calls" set is now fully resolved: missing `<summary>` tags, a `file`-modifier SA1400 false positive worked around via `internal`, `MockLogger`→`_mockLogger` casing ×3, a missing file header, stray blank lines before closing braces, and one embedded trailing comment) | ✅ Frontend `tsc --noEmit` clean, `CI=true npm run build` succeeds | **Tests:** ✅ 7,517 + 2,907 = **10,424 backend tests passing, 0 failures** (re-verified fresh Aug 7 2026 after round 5) | ✅ **1,134 of 1,137 frontend tests passing** (46 suites) — 3 failures are pre-existing flakiness in `IntegrationsSettingsPage.test.tsx` (untouched by rounds 4-5, from an earlier round), confirmed to pass 5/5 in isolation; timeout-related under full-suite parallel load, not a regression | **Coverage:** ~70% (TCOV-001–068 done) — still overstates method-level completeness for AccountService/LeadsController/WorkflowController/AllenAIService (unaddressed) | **Next:** nothing decision-gated remains in Section 2B; see the two explicitly-deferred Lead-migration follow-ups (REM-LEAD-BACKFILL-EXEC, REM-LEAD-HISTORY-CONTINUITY) below
>
...
| REV-BUG-006 | ⚠️ **Partially fixed** (`tech-debt-cleanup-aug2026`, round 4). Decision: fix the no-op bug now, defer the outbound-integration decision. The webhook now fails closed — rejects with 401 when `STRIPE_WEBHOOK_SECRET` is unconfigured instead of silently skipping signature verification (13 tests, including 2 new covering this case). `StripeIntegrationService`'s outbound calls (create payment intent, charge, capture, refund) remain **100% simulated** — still open, still needs a real Stripe.NET integration decision; not attempted (see Section 2B Remediation Plan). |
...
| REM-BUG-006 | ⚠️ Partially fixed (round 4) | P2 | L | Decision: fix the no-op bug now (done — see REV-BUG-006), defer the outbound Stripe.NET integration decision. Still needs: commit to real Stripe.NET integration for outbound calls, or explicitly document Stripe as inbound-webhook-only. | **Yes, remains** — real Stripe wiring is a scoped effort of its own (sandbox credentials, PCI considerations); shouldn't be attempted piecemeal inside an unrelated task. |
...
| REM-LEAD-HISTORY-CONTINUITY | ❌ Open (new, round 4) | P3 | M | Activities/RecordComments made against the old Contact-based lead records are not re-parented onto the new Lead IDs after backfill — `Lead.ContactId` preserves an audit back-reference, but old activity/comment history stays attached to the original Contact record rather than surfacing on the new Lead. Proportionate scope call for this cleanup pass, not a silent gap. | Soft — decide whether this history needs to be visible on the migrated Lead record, or whether the Contact-ID back-reference is sufficient for audit purposes. |
...
**Sequencing:** as of the round-4 pass, all 5 previously decision-gated items are closed. The user was asked for a decision on each (via research-backed multiple-choice questions, not a blind agent call) before any code was written — REM-ORPHAN-001/002 were preceded by dedicated comparison research passes (reading all competing implementations, git history, and — for Change Management — confirming the frontend already called routes only the "losing" implementations exposed) specifically so the decision wasn't made blind. What remains open: REM-ORPHAN-005's Twilio half (blocked on a webhook endpoint that doesn't exist, unchanged from round 3), REM-BUG-006's outbound Stripe integration (deferred by explicit user choice, no-op bug fixed), and two new round-4 follow-ups — REM-LEAD-BACKFILL-EXEC (tooling built, execution against real data deliberately not attempted) and REM-LEAD-HISTORY-CONTINUITY (a documented scope tradeoff, not a silent gap).
