...
> **Last Updated:** August 6, 2026 (full end-to-end review — see [Section 2A](#section-2a--engineering-review-findings-august-6-2026) — followed by remediation work on `tech-debt-cleanup-aug2026`, see [Section 2B](#section-2b--findings-discovered-while-implementing-section-2a-august-6-2026))
> **Version:** 0.625.9 (branch `tech-debt-cleanup-aug2026`, not yet merged to `main`)
...
> **Active Backlog:** 1 blocked (INT-003) + 1 deferred by architectural decision (XMOD-011) + Section 2A (27 items from the Aug 6 review — **9 fixed/mostly-fixed** on `tech-debt-cleanup-aug2026`, see status markers, 18 still open) + **Section 2B (19 new items found while implementing 2A — 2 fixed, 17 open with a full remediation plan, including a 5-instance "tested service the live API never calls" pattern — REV-ORPHAN-\*)**
> **Build:** ✅ 0 backend build errors | ⚠️ **9 live StyleCop SA warnings** (42 total incl. non-SA) — down from 255/286 after a mechanical cleanup pass on `tech-debt-cleanup-aug2026`; not zero, re-verified with `dotnet build CRM.sln --no-incremental`, Aug 6 2026 | ✅ Frontend `tsc --noEmit` clean (0 errors) and `npm run build` now succeeds under `CI=true` too (REV-BUILD-002 fixed) | **Tests:** ✅ 2,853 passing in CRM.Tests, 0 failures, 5 skipped (up from 2,785 — 68 new tests added across the fixes; re-verified by running `dotnet test` directly, Aug 6 2026) | **Coverage:** ~70% (TCOV-001–068 done), TCOV2 Waves A–E complete (v0.625.0) — ⚠️ still overstates method-level completeness for AccountService/LeadsController/WorkflowController/AllenAIService; MergeService's real gap closed (REV-DOC-005 partial) | **Next:** Measure actual % with reportgenerator; work through the Section 2B remediation plan, starting with the no-decision-needed items
...
**Recommended next actions:** triage remaining REV-STUB-* and REV-FE-* into the phased plan (most are P1/P2 — high-value but non-blocking stubs), schedule the remaining 9-entity FIELD_GAP_REMEDIATION_PLAN.md re-audit (REV-DOC-003), and — now the higher priority given Section 2B — decide the fate of each orphaned-but-tested implementation (REV-ORPHAN-\*) before adding any more parallel logic to those modules.
...
| REV-ORPHAN-004 | `QuotesController` (the live `/api/quotes` API) never calls `IQuoteService`/`QuoteService` — it talks to `CrmDbContext` directly and reimplements CRUD/lifecycle itself, including bypassing status-transition guards via direct field assignment. `QuoteService` is fully implemented with 44 passing tests (TCOV2-A06); its only real consumers are an AI plugin and a webhook controller, not the REST API. Compounding this: see REV-TESTFAKE-001 below. | SPEC-SLS-003-QuoteManagement.md |
| REV-ORPHAN-005 | Neither the generic `SlackNotificationService`/`TeamsNotificationService` (fixed in REV-STUB-009) nor their "real" siblings (`TeamsNotificationChannelService`, ITSM Slack/Teams services) have any DI registration or caller anywhere in the codebase. Same for `TwilioCallLoggingService` (REV-STUB-001) after its fix. Correctly implemented now, still unreachable at runtime. | Commits on `tech-debt-cleanup-aug2026`, this section |

### REV-TESTFAKE-\* — Test files that provide zero real coverage despite existing

| ID | Description | Source |
|----|---|---|
| REV-TESTFAKE-001 | `QuotesControllerTests.cs` — the file that should test the live `/api/quotes` controller — contains only `Assert.True(true)` placeholders. Effectively zero coverage of the actual API path, while the untested-in-practice `QuoteService` (see REV-ORPHAN-004) has 44 tests nobody's production traffic exercises. | SPEC-SLS-003-QuoteManagement.md |
...
| REV-BUG-003 | `MergeService.GetRecordSnapshotAsync` does a bare `JsonSerializer.Serialize(record)` on the raw EF entity with no cycle handling. If other tracked entities in the same `DbContext` create a circular navigation reference (e.g. `Lead.Opportunities` ↔ `Opportunity.Lead`), snapshot serialization throws and the whole merge fails. | ❌ Open — found while writing MergeService tests, explicitly out of scope for that fix. |
| REV-BUG-004 | `OrderService.CreateFromQuoteAsync` calls `.Include(q => q.LineItems)` where `Quote.LineItems` is actually a scalar `string` property, not the real navigation property (`QuoteLineItems`). Likely throws at runtime; invisible in tests because all `OrderService` tests mock `IOrderService`. | ❌ Open. |
| REV-BUG-005 | Contract module has several confirmed frontend/backend mismatches: the Renew endpoint ignores its request body; `contractService.ts` calls `/signature` (doesn't exist) instead of the real `/send-for-signature` + `/signature-status`; frontend field names `title`/`contractStatus` don't match backend `name`/`status`, breaking `ContractDetailsPage.tsx` rendering; `ContractsPage.tsx` calls `/upload`/`/download` routes that don't exist at all; three divergent frontend `Contract` type definitions coexist. | ❌ Open. |
| REV-BUG-006 | `StripeIntegrationService`'s outbound calls (create payment intent, charge, capture, refund) are **100% simulated** — no Stripe.NET SDK call anywhere, confirmed by the test file's own comment — despite being marked TCOV2-B01 ✅. Inbound webhook handling (`StripeWebhookController`) is real (HMAC-SHA256 verified). This is why REV-STUB-012/013's card-charge step can't be closed yet. Also: Stripe webhook signature verification silently no-ops when `STRIPE_WEBHOOK_SECRET` is unset. | ❌ Open — needs a real Stripe.NET integration decision, out of scope for a cleanup pass. |
| REV-BUG-007 | `territoryService.ts`'s "Auto-Assign" button calls a route that doesn't exist on the backend. Three separate frontend `OrderStatus` enums exist, one (`types/sales.ts`) with numeric values that don't match the backend at all — contradicts `FIELD_GAP_REMEDIATION_PLAN.md`'s "Complete"/"mapped" claim for Order. | ❌ Open. |
| REV-BUG-008 | `CrmTask` write DTOs (`CreateCrmTaskDto`/`UpdateCrmTaskDto`) still lack `TaskType`/`StartDate` (update also lacks `AccountId`/`OpportunityId`), even though the read DTO now has all 5 after REV-FGAP-002. Narrower, separate gap — a task's type/start date can be read back but not set via the API. | ❌ Open. |
| REV-BUG-009 | `Contact`: `PreferredContactMethod` is accepted in request DTOs but silently dropped — never written to the entity. `Update`/`Delete`/social-media endpoints have zero unit-test coverage at both service and controller layers. Account-assignment methods exist on `ContactsService` but have no controller route. | ❌ Open. |
| REV-BUG-010 | `TasksController` bypasses `TaskService`/`ITaskService` entirely — queries `CrmDbContext` directly with its own duplicate mapping logic. The two paths disagree on delete semantics: the controller hard-deletes, the service soft-deletes. | ❌ Open — related to, but distinct from, REV-ORPHAN-\*: here the orphaned side (`TaskService`) is the one with correct soft-delete semantics. |
| REV-BUG-011 | `SalesForecastService`/`SalesForecastsController` are solid and tested on the backend, but a repo-wide search confirms **zero frontend implementation** — no page, service, or route anywhere references `SalesForecast` or `/api/sales-forecasts`. | ❌ Open — needs a frontend feature, not a wire-up. |
...
| ID | Priority | Effort | Proposed Remediation | Decision Needed Before Starting |
|----|----------|--------|------------------------|----------------------------------|
| REM-ORPHAN-001 | P1 | L | Pick ONE Change Management implementation to be canonical. Likely candidate: wire `/api/changes` to `ChangeManagementService`/`CABWorkflowService` (already tested) instead of the untested `ChangeService`; retire the other three (`ChangeService`, `IChangeManagementServiceEx`) — or the reverse if `ChangeService` turns out to have functionality the tested pair lacks. Read all four before choosing. | **Yes** — which of the 4 implementations is functionally correct/current; a code review can't tell alone which one reflects the actual desired business process. |
| REM-ORPHAN-002 | P1 | L | Same shape as REM-ORPHAN-001: consolidate `CommissionRulesEngine`/`CommissionRuleService`/`CommissionRuleEvaluationService` into one, wire `CommissionsController` to it; decide whether `TerritoryAssignmentService` replaces or complements whatever `TerritoriesController` uses today. | **Yes** — which commission engine encodes the current real commission plan rules; likely needs Sales-ops sign-off, not just an engineering call. |
| REM-ORPHAN-003 | P2 | L | Decide product direction: either (a) migrate the Contacts-as-Leads UI onto the richer `Lead`/`LeadService` system and retire the JSON-in-notes approach, or (b) formally retire the standalone `Lead` system and fold any features worth keeping (BANT/MEDDIC, nurture, UTM) into the Contact-based flow. Both are real migrations, not quick fixes. | **Yes** — this is a product/UX decision (which lead model users should see), not something to infer from code alone. |
| REM-ORPHAN-004 | P1 | M | Wire `QuotesController` to `QuoteService` (bringing its status-transition guards and 44 tests to the live API), replacing the controller's direct `CrmDbContext` CRUD. Smaller/safer than REM-ORPHAN-001/002 since `QuoteService` already looks like the more correct implementation — but must be paired with REM-TESTFAKE-001 so the switch is actually verified, not just assumed to work. | Low — mostly an engineering call, but confirm no `QuotesController`-only behavior (e.g. the AI-plugin/webhook integration paths) would break by consolidating onto `QuoteService`. |
| REM-ORPHAN-005 | P2 | S | Register `ISlackNotificationService`/`ITeamsNotificationService`/`ITwilioCallLoggingService` in DI and wire at least one real caller each (e.g. workflow/escalation notification steps, ticket call logging) so the now-correct implementations are actually reachable. | No — pure wiring, now that REV-STUB-001/009 made the implementations real. |
| REM-TESTFAKE-001 | P0 | S | Rewrite `QuotesControllerTests.cs` with real assertions against the live controller (status codes, service/DbContext interaction, validation) before or alongside REM-ORPHAN-004 — don't let a "tests exist" checkmark keep hiding zero real coverage on a live billing-adjacent API. | No — pure test-authoring, but do it before relying on `QuotesControllerTests.cs` passing as any signal. |
| REM-BUG-003 | P2 | S | Add cycle-safe serialization to `MergeService.GetRecordSnapshotAsync` (e.g. `JsonSerializerOptions.ReferenceHandler = Preserve`, or project to a flat snapshot DTO instead of serializing the raw tracked entity). | No. |
| REM-BUG-004 | P1 | S | Fix `OrderService.CreateFromQuoteAsync`'s `.Include(q => q.LineItems)` to reference the real `QuoteLineItems` navigation property; add a test that exercises the actual `OrderService` implementation (not a mock) for this path so the bug can't hide behind mocked tests again. | No — clear bug, `Quote.LineItems` vs `QuoteLineItems` is unambiguous from the entity definition. |
| REM-BUG-005 | P1 | M | Fix the 4 confirmed Contract frontend/backend mismatches one at a time: (1) Renew endpoint honor its request body, (2) `contractService.ts` call the real signature routes, (3) rename FE fields to match backend (`name`/`status`) or add explicit mapping, (4) either implement `/upload`/`/download` on the backend or remove the FE calls. Also collapse the three divergent frontend `Contract` types into one. | No — each is independently verifiable against the real controller routes. |
| REM-BUG-006 | P2 | L | Product decision: commit to a real Stripe.NET integration for outbound calls (payment intent/charge/capture/refund) or explicitly document Stripe as inbound-webhook-only for now. Either way, fix the silent no-op when `STRIPE_WEBHOOK_SECRET` is unset (should fail closed/loud, not silently skip verification). | **Yes** — real Stripe wiring is a scoped effort of its own (sandbox credentials, PCI considerations); shouldn't be attempted piecemeal inside an unrelated task. |
| REM-BUG-007 | P2 | S | Fix or remove `territoryService.ts`'s "Auto-Assign" button's dead route call. Consolidate the three frontend `OrderStatus` enums into one sourced from the backend's actual numeric values (this is exactly what the DTO-superset contract test pattern from REV-FGAP-001/002 is meant to catch — extend that pattern to enums). | No. |
| REM-BUG-008 | P2 | S | Add `TaskType`/`StartDate` to `CreateCrmTaskDto` and `TaskType`/`StartDate`/`AccountId`/`OpportunityId` to `UpdateCrmTaskDto`, plus mapper + controller wiring, mirroring the REV-FGAP-002 pattern exactly. | No. |
| REM-BUG-009 | P2 | M | Wire `PreferredContactMethod` from the request DTO into the `Contact` entity in `ContactsService`. Add missing unit tests for `Update`/`Delete`/social-media endpoints. Add a controller route for the existing account-assignment service methods, or remove the dead methods if they're superseded elsewhere — check first. | No. |
| REM-BUG-010 | P0 | M | Decide and fix the delete-semantics inconsistency first (hard-delete via controller vs soft-delete via service is a data-loss risk, not just an architecture smell) — likely means switching `TasksController` to soft-delete like the rest of the codebase, then wiring the controller to `TaskService` to remove the duplicate mapping logic entirely. | Low — soft-delete is this codebase's established convention (`BaseEntity.IsDeleted`), so the correct direction is fairly clear, but confirm no caller depends on hard-delete behavns. |
| REM-BUG-011 | P3 | L | Build a Sales Forecasting frontend (list/detail view, likely a dashboard widget) against the already-solid `SalesForecastsController` API. Net-new UI work, not a wire-up. | **Yes** — needs product/design input on what the forecasting UI should look like; nothing to copy from an existing partial implementation. |

**Sequencing:** REM-TESTFAKE-001, REM-BUG-004, REM-BUG-005, REM-BUG-008, and REM-ORPHAN-005 are safe to pick up immediately (no decision blocking them). REM-BUG-010 is P0 due to the data-loss risk but needs a quick confirmation, not a lengthy decision. REM-ORPHAN-001/002/003 and REM-BUG-006/011 need explicit sign-off (architecture or product) before an agent should touch them — attempting them blind risks silently deleting the *better* implementation, which would make this exact class of problem worse, not better.
