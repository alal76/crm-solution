...
def day2_components_status():
    # Return current provider selections + config from profile + container status
...
def day2_components_configure():
    # Save provider selection + config (API keys, URLs) to the active profile
...
def day2_components_apply():
    ...
    deploy_dir = (
        profile.get("deploy_dir")
        or str(Path.home() / CDT_DIR / "deployments" / pname)
    )
